...
var test ChunkInfo
    ...
    test=ChunkInfo{
	   ...
	   chunk_info_list=append(chunk_info_list,test)
	}

	var chunk_info_map map[int]ChunkInfo
	chunk_info_map = make(map[int]ChunkInfo)

	var min_chunk_id int = chunk_info_list[0].Id
	for _, one_chunk := range chunk_info_list {
	chunk_info_map[one_chunk.Id] = one_chunk
	fmt.Println(one_chunk.Id,chunk_info_map[one_chunk.Id])
		if one_chunk.Id < min_chunk_id {
			min_chunk_id = one_chunk.Id
		}
	}

	fmt.Println(min_chunk_id,chunk_info_map)
...
def get_spdk_rpc_addr(config_file):
    config = ConfigParser.RawConfigParser()
    config.read(config_file)
    try:
        sock_addr = config.get("common", "spdk_rpc_addr")
    except Exception as e:
        logger.warning("config file don't have spdk_rpc_addr configuration, use default /var/tmp/spdk.sock instead. error message: %s", e)
        sock_addr = "/var/tmp/spdk.sock"
    logger.info("spdk rpc sock_addr: %s" , sock_addr)
    return sock_addr

def get_restore_file(config_file):
    config = ConfigParser.RawConfigParser()
    config.read(config_file)
    try:
        file_dir = config.get("common", "restore_file")
    except Exception as e:
        logger.warning("config file don't have restore_file configuration, use default /var/tmp/vhost.conf instead. error message: %s", e)
        file_dir = "/var/tmp/vhost.conf"
    logger.info("restore file dir: %s" , file_dir)
    return file_dir
